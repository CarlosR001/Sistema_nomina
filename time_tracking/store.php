...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_periodo_reporte = $_POST['id_periodo_reporte'] ?? null; // Nuevo: Recoger el ID del período
    ...
    // Construir la URL de redirección base para mantener el contexto del período
    ...
    try {
        $sql_insert = "INSERT INTO RegistroHoras (id_contrato, id_proyecto, id_zona_trabajo, fecha_trabajada, hora_inicio, hora_fin, estado_registro, id_periodo_reporte) 
                       VALUES (:id_contrato, :id_proyecto, :id_zona, :fecha, :inicio, :fin, 'Pendiente', :id_periodo)";
        ...
        $stmt_insert->execute([
            ...
            ':inicio' => $hora_inicio, 
            ':fin' => $hora_fin,
            ':id_periodo' => $id_periodo_reporte // Nuevo: Guardar el ID del período
        ]);
