...
try {
    ...
    $sql = "UPDATE contratos SET id_posicion = ?, tipo_contrato = ?, tipo_nomina = ?, frecuencia_pago = ?, fecha_inicio = ?, fecha_fin = ?, salario_mensual_bruto = ?, tarifa_por_hora = ?, estado_contrato = ?, permite_horas_extras = ?, horario_entrada = ?, horario_salida = ? WHERE id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id_posicion, $tipo_contrato, $tipo_nomina, $frecuencia_pago, $fecha_inicio, $fecha_fin, $salario_mensual_bruto, $tarifa_por_hora, $estado_contrato, $permite_horas_extras, $horario_entrada, $horario_salida, $id]);
