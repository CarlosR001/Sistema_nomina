<?php
// payroll/procesar_horas.php - v4.4 (Corrección de Lógica de Lugar Definitiva)
...
function calcular_horas_nocturnas_reales(DateTime $inicio_turno, DateTime $fin_turno) {
    ...
    $hora_fin_nocturna = 7;
    while ($cursor < $fin) {
        $hora_actual = (int)$cursor->format('G');
...
try {
    ...
    // --- CORRECCIÓN FINAL Y DEFINITIVA DE LA CONSULTA ---
    // Ahora se selecciona rh.id_zona_trabajo directamente.
    $sql_horas = "SELECT 
                    c.id as contrato_id, c.tarifa_por_hora, 
                    e.nombres, e.primer_apellido, 
                    rh.fecha_trabajada, rh.hora_inicio, rh.hora_fin, 
                    rh.id_zona_trabajo, -- <-- ESTE ES EL CAMBIO CLAVE
    ...
    foreach ($horas_aprobadas as $contrato_id => $registros) {
        ...
        $total_horas_nocturnas_bono = 0; 
        $total_horas_gracia = 0;
        ...
        foreach ($registros as $reg) {
            if ($reg['transporte_aprobado'] && !empty($reg['id_zona_trabajo']) && isset($zonas[$reg['id_zona_trabajo']])) {
                ...
                if ($reg['transporte_mitad']) {
                    $pago_transporte_total += ($costo_lugar * 0.5);
                } else {
    ...
    elseif ($mode === 'final') {
        ...
        $codigos_automaticos = ['ING-NORMAL', 'ING-EXTRA', 'ING-FERIADO', 'ING-NOCTURNO', 'ING-TRANSP', 'ING-GRACIA'];
        ...
        $sql_delete_auto = "DELETE np FROM novedadesperiodo np JOIN conceptosnomina cn ON np.id_concepto = cn.id WHERE np.id_contrato IN ($contratos_placeholders) AND np.periodo_aplicacion BETWEEN ? AND ? AND cn.codigo_concepto IN ($codigos_auto_placeholders)";
        $stmt_delete_auto = $pdo->prepare($sql_delete_auto);
        $stmt_delete_auto->execute(array_merge($contratos_a_procesar, [$fecha_inicio, $fecha_fin], $codigos_automaticos));
        ...
        foreach ($results as $contrato_id => $res) {
            ...
            if ($res['pago_gracia'] > 0) $stmt_insert->execute([$contrato_id, $conceptos['ING-GRACIA'], $fecha_inicio, $res['pago_gracia']]);
        }

        foreach ($novedades_manuales_rescatadas as $novedad_manual) {
            $stmt_insert->execute([ $novedad_manual['id_contrato'], $novedad_manual['id_concepto'], $novedad_manual['periodo_aplicacion'], $novedad_manual['monto_valor'], 'Pendiente' ]);
        }
